...
import { Plus, Dumbbell, Target, CheckSquare, Square, Flame } from 'lucide-react';
      ...
      {showForm && (
        <div className="rounded-xl p-4 border space-y-3" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))}
                placeholder="Push-ups, Running..." className="w-full px-3 py-2 rounded-lg text-sm outline-none"
          ...
          <div className="flex gap-2">
            <button onClick={addLog} className="px-4 py-2 rounded-lg text-sm font-medium text-white" style={{ background: 'var(--color-accent)' }}>Save</button>
      ...
      {tab === 'logs' && (
        <div className="space-y-2">
          {logs.length === 0
            ...
            : logs.map(l => (
              <div key={l.id} className="flex items-center gap-3 p-3 rounded-xl border cursor-pointer"
                ...
                <div className="flex-1">
                  <div className={l.completed ? 'line-through text-sm' : 'text-sm'} style={{ color: l.completed ? 'var(--color-muted)' : 'var(--color-text)' }}>{l.title}</div>
                  <div className="text-xs" style={{ color: 'var(--color-muted)' }}>
                    {[l.duration, l.intensity, l.calories ? `${l.calories} cal` : null].filter(Boolean).join(' · ')}
                  </div>
                </div>
                {l.calories && <span className="text-xs flex items-center gap-1" style={{ color: '#ef4444' }}>
